...
namespace Headstone.Service
{
    ...
    {
        private BasketServiceBase _basketServiceBase = new BasketServiceBase();
        ...
        public BasketServiceResponse<Basket> DeleteBasket(BasketDeleted ev, List<ServiceLogRecord> logRecords = null)
        {
            // Create the watch
            var sw = new Stopwatch();
            sw.Start();
            ...
            logRecords.Add(new ServiceLogRecord()
            {
                Type = "DEBUG",
                TimeStamp = DateTime.Now,
                Body = "Basket delete request received."
            });
            ...
            if (ev.BasketID == default(int))
            {
                dataErrors.Add("No valid Basket id found!");
            }
            ...
            Basket Basket = _basketServiceBase.Get(r => r.BasketID == ev.BasketID).Result.FirstOrDefault();
            ...
            if (Basket == null)
            {
                ...
                logRecords.Add(new ServiceLogRecord()
                {
                    Type = "DEBUG",
                    TimeStamp = DateTime.Now,
                    Body = "No Basket found with the given id!"
                });
                ...
                response.Code = ((short)HeadstoneServiceResponseCodes.Invalid_Request).ToString();
                response.ServiceTook = sw.ElapsedMilliseconds;
                response.Message = "No Basket found with the given id!";
                response.Errors.Add("No Basket found with the given id!");
                response.LogRecords = logRecords;
            ...
            logRecords.Add(new ServiceLogRecord()
            {
                Type = "DEBUG",
                TimeStamp = DateTime.Now,
                Body = "Deleting Basket."
            });

            // Delete the billing info
            var baseServiceResponse = _basketServiceBase.Delete(Basket);
            ...
            if (baseServiceResponse.Type != Headstone.Framework.Models.ServiceResponseTypes.Success)
            {
                ...
                logRecords.Add(new ServiceLogRecord()
                {
                    Type = "ERROR",
                    TimeStamp = DateTime.Now,
                    Body = "There was an error while deleting the Basket!"
                });
                ...
                response.Code = ((short)HeadstoneServiceResponseCodes.General_Exception).ToString();
                response.ServiceTook = sw.ElapsedMilliseconds;
                response.Message = "There was an error while deleting the Basket!";
                response.Errors.Add("There was an error while deleting the Basket!");
                response.Errors.AddRange(baseServiceResponse.Errors);
                response.LogRecords = logRecords;
            ...
            else
            {
                ...
                logRecords.Add(new ServiceLogRecord()
                {
                    Type = "DEBUG",
                    TimeStamp = DateTime.Now,
                    Body = string.Format("Basket successfuly deleted. BasketID:{0}",
                                            Basket.BasketID)
                });

                // Add the new object to the result
                response.Result.Add(Basket);
            ...
            response.Code = ((short)HeadstoneServiceResponseCodes.Request_Successfuly_Completed).ToString();
            response.ServiceTook = sw.ElapsedMilliseconds;
            response.Message = string.Format("Basket successfuly deleted. BasketID:{0}",
                                            Basket.BasketID);
            response.LogRecords = logRecords;
